...
class ARCViT(eqx.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.patch_embed = PatchEmbed(
            image_size=image_size, patch_size=patch_size, embed_dim=embed_dim, key=patch_key
        )
        self.positional_embed = jax.random.normal(
        ...
        self.head = eqx.nn.Linear(
            embed_dim, num_colors * (patch_size**2), key=head_key
        )
    ...
    def _token_mask(self, attention_mask: jax.Array) -> jax.Array:
        if attention_mask.shape != (self.image_size, self.image_size):
            raise ValueError(
                f"`attention_mask` must be {(self.image_size, self.image_size)}, got {attention_mask.shape}."
            )
        ...
        return jnp.concatenate([prefix, flat], axis=0)
    ...
    def __call__(
        ...
        *,
        ...
        embedded = color_lookup(pixels)  # (H, W, embed_dim)
        embedded = jnp.transpose(embedded, (2, 0, 1))  # (embed_dim, H, W)
        patch_tokens = self.patch_embed(embedded) + self.positional_embed

        task_tokens = self.task_token_embed(task_id).reshape(
        ...
        token_mask = self._token_mask(attention_mask) if attention_mask is not None else None
        encoded = self.encoder(
            hidden_states, mask=token_mask, key=enc_key, inference=inference
        )
        norm_all = jax.vmap(self.norm)
        encoded = norm_all(encoded)
        patch_states = encoded[self.num_task_tokens :, :]

        logits = jax.vmap(self.head)(patch_states)  # (seq, num_colors * patch_size^2)
        ...
        logits = jnp.transpose(logits, (2, 0, 1))  # (C, H, W)
        return logits
